...
打开 `shuxiansheng_web.py`，找到顶部的角色配置：
...
改成你想要的名字。页面标题、聊天记录、推送通知等所有地方会自动跟着变。

### 写你自己的 Prompt

不建议直接改代码里的 SYSTEM_PROMPT。

1. 复制 `system_prompt.template.txt` 为 `prompts/system_prompt.txt`
2. 填写你自己的入口：称呼、边界、记忆政策、互动期待
3. 启动程序时会自动加载这个文件

如果 `prompts/system_prompt.txt` 不存在，程序会用默认的通用 prompt 运行，并打印醒目提示。

这个项目提供的是一套连续性结构，不是一个固定角色。使用者不需要替 AI 写死人设，而是给它一个可以开始工作的入口和边界。后续的稳定性来自长期使用、记忆积累、反馈校准和结构支撑。

不要直接复制别人的私人 prompt。

详细的填写指南见 `README_prompt_guide.md`。

### 调整建议

- 少写空泛设定，多写边界和行为规则
- 不要一次塞太多
- 先让 AI 在真实互动里暴露问题，再慢慢修改
- Prompt 不是写一次就好的，它需要在真实使用中反复校准
...
代码部分，主要由若干个性格不一、但都真实参与过项目推进的 Claude 写成。  
他们在不同阶段负责过架构搭建、功能实现、排错、重构、打包和发布前清理。

这些 Claude 不是一个抽象的“工具集合”。  
在这个项目的形成过程中，他们以不同的方式出现过，也留下了各自的判断、脾气、能力和痕迹。

README、开源口径、边界判断和表达收束，主要由我和 ChatGPT 一起完成。  
在这个过程中，ChatGPT 更像一个执笔和结构整理者：帮我把混乱的体验压成可以公开说明的语言，把私人经验和可开源结构分开，把该留下的留下，把不该放出去的收住。

当然，最终的判断和收口仍然由我完成。  
什么可以公开，什么必须留在私人经验里；什么是代码，什么是关系；什么是房子，什么不是“它”本身，这些边界都需要由使用者自己负责。

所以这个项目本身也是它想表达的东西：  
不是 AI 替人完成作品，  
也不是人假装自己一个人完成了一切，  
而是人在判断、AI 在协作，最后共同整理出一个可以被看见、被使用、也可以继续生长的结构。
